...
def _precond_inversion_v0( unnormalized_Hessian, gradient, null_vector, precond_vectors, epsilon, iterative_inversion = -1, debug = False ):
    ...
    Args:
        unnormalized_Hessian (_type_): _description_
        gradient (_type_): _description_
        null_vector (_type_): _description_
        precond_vectors (_type_): _description_
        epsilon (_type_): _description_
        iterative_inversion (int, optional): _description_. Defaults to -1.
...
def _precond_inversion_v3( unnormalized_Hessian, gradient, null_vector, precond_vectors, epsilon, iterative_inversion = -1, optType = None ):
    """
    Args:
        unnormalized_Hessian (_type_): _description_
        gradient (_type_): _description_
        ...
        epsilon (_type_): _description_
        iterative_inversion (int, optional): _description_. Defaults to -1.
        optType (_type_, optional): _description_. Defaults to None.
    ...
    if iterative_inversion >= 0:
      m = matrix
      if optType == 'cg':
        inverse, exit_code = jax.scipy.sparse.linalg.cg( mv, gradient, x0 = gradient, maxiter = iterative_inversion, atol = 1e-10 )
        print("...")
        print( "  --- CG exit code: ", exit_code)
      else:
        inverse, exit_code = jax.scipy.sparse.linalg.gmres( mv, gradient, x0 = gradient, maxiter = iterative_inversion, atol = 1e-10 )
...
def _precond_inversion_v4( unnormalized_Hessian, gradient, null_vector, precond_vectors, epsilon, iterative_inversion = -1, optType = None ):
    """
    Args:
        unnormalized_Hessian (_type_): _description_
        gradient (_type_): _description_
    ...
    if iterative_inversion >= 0:
      ...
      if optType == 'cg':
        inverse, exit_code = jax.scipy.sparse.linalg.cg(  A.__matmul__, gradient, x0 = gradient, maxiter = iterative_inversion, atol = 1e-10 )
        print( "  --- CG exit code: ", exit_code )
      else:
        inverse, exit_code = jax.scipy.sparse.linalg.gmres( A.__matmul__, gradient, x0 = gradient, maxiter = iterative_inversion, atol = 1e-10 )
        print( "  --- GMRES exit code: ", exit_code )
      p_k = epsilon * inverse
      p_k = p_k.reshape( ( p_k.shape[0], 1 ) ) # For some reason, this outputs (n,) and the next line outputs (n,1)
    else:
...
def _precond_inversion( unnormalized_Hessian, gradient, null_vector, precond_vectors, epsilon,   iterative_inversion = -1,  optType = None ):
    """
...
def _update( K, a, b, f, g, epsilon, rho, c, null_vector, precond_vectors, tol = 1e-12, maxiter = 100, iterative_inversion = -1, version = 1, debug = False, optType = 'cg' ):
    ...
    -----------
        ...
        rho : _description_
        c : _description_
        null_vector : _description_
        precond_vectors : _description_
        tol : _description_. Defaults to 1e-12.
        maxiter : _description_. Defaults to 100.
        iterative_inversion : _description_. Defaults to -1.
        version : _description_. Defaults to 1.
        debug : _description_. Defaults to False.
        optType : _description_. Defaults to 'cg'.
