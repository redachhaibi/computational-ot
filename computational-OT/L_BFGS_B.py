...
class L_BFGS_B:
    ...
    def _computegradient(self,x):
        ...
        gradient= np.vstack((-(self.a-(np.exp(x[:self.a.shape[0]]/self.epsilon)*np.dot(self.K,np.exp(x[self.a.shape[0]:]/self.epsilon))).reshape(x[:self.a.shape[0]].shape[0],-1)),-(self.b-(np.exp(x[self.a.shape[0]:]/self.epsilon)*np.dot(self.K.T,np.exp(x[:self.a.shape[0]]/self.epsilon))).reshape(x[self.a.shape[0]:].shape[0],-1))))
        return gradient

    def _objectivefunction(self,x):
        """Computes the value of the objective function at x"""
     
        return -(np.dot(x[:self.a.shape[0]].T,self.a)+np.dot(x[self.a.shape[0]:].T,self.b)-self.epsilon*np.dot(np.exp(x[:self.a.shape[0]]/self.epsilon).T,np.dot(self.K,np.exp(x[self.a.shape[0]:]/self.epsilon))))
    ...
    def _update(self, tol=1e-12, maxiter=1000,pgt=1e-20,maxl=100,factor=100000000.0):
        ...
        # end for  
        return self.x[:self.a.shape[0]],self.x[self.a.shape[0]:],self.err_a,self.err_b ,self.objvalues
